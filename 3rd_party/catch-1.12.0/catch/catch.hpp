    ...
    namespace Detail {
        ...
        public:
            ...
            friend bool operator ==(const T &lhs, Approx const &rhs) {
                ...
                bool relativeOK = std::fabs(lhs_v - rhs.m_value) < rhs.m_epsilon * (rhs.m_scale + (std::max)(
                                      std::fabs(lhs_v), std::fabs(rhs.m_value)));
                if (relativeOK) {
                    return true;
                }
            ...
            friend bool operator ==(double lhs, Approx const &rhs) {
                // Thanks to Richard Harris for his help refining this formula
                bool relativeOK = std::fabs(lhs - rhs.m_value) < rhs.m_epsilon * (rhs.m_scale + (std::max)(
                                      std::fabs(lhs), std::fabs(rhs.m_value)));
                if (relativeOK) {
                    return true;
                }
...
namespace Catch {
    ...
    public:
        TestSpecParser(ITagAliasRegistry const &tagAliases) : m_mode(None), m_exclusion(false), m_start(0), m_pos(0),
...
namespace Catch {
    namespace TestCaseTracking {
        ...
        public:
            ...
            void tryOpen() {
                if (!isComplete() && (m_filters.empty() || m_filters[0].empty() || m_filters[0] == m_nameAndLocation.
                                      name))
                    open();
            }
...
namespace Catch {
    AssertionInfo::AssertionInfo() : macroName(""), capturedExpression(""),
    ...
    std::string AssertionResult::getExpressionInMacro() const {
        if (m_info.macroName[0] == 0)
            ...
            return std::string(m_info.macroName) + "( " + capturedExpressionWithSecondArgument(
...
namespace Catch {
    namespace Matchers {
        namespace StdString {
            ...
            ContainsMatcher::ContainsMatcher(CasedString const &comparator) : StringMatcherBase(
                "contains", comparator) {
            }
...
namespace Catch {
    struct ConsoleReporter : StreamingReporterBase {
        ...
        virtual void sectionEnded(SectionStats const &_sectionStats) CATCH_OVERRIDE {
            ...
            if (m_config->showDurations() == ShowDurations::Always) {
                stream << getFormattedDuration(_sectionStats.durationInSeconds) << " s: " << _sectionStats.sectionInfo.
                        name << std::endl;
            }
            if (m_headerPrinted) {
                m_headerPrinted = false;
