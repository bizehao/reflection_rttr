...
/// @mainpage
/// [ChaiScript](http://www.chaiscript.com") is a scripting language designed specifically for
/// integration with C++. It provides seamless integration with C++ on all levels, including
/// shared_ptr objects, functors and exceptions.
///
/// The parts of the ChaiScript API that the average user will be concerned with are contained in
/// the chaiscript namespace and the chaiscript::ChaiScript class.
///
...
///
/// To compile the above application on a Unix like operating system (MacOS, Linux) with GCC you
/// need to link the dynamic loader. For example:
///
/// ~~~~~~~~
...
/// std::string result = chai.eval_file<std::string>("myfile.chai") // extract the last value
...
/// chai.add(fun<void (MyClass::*)(const std::string &)>(&MyClass::overloadedmethod),
/// "overloadedmethod");
/// ~~~~~~~~~
///
/// There are also shortcuts built into chaiscript::fun for binding up to the first two parameters
/// of the function.
///
/// ~~~~~~~~~{.cpp}
/// MyClass obj;
...
///
/// Operators are just like any other function in ChaiScript, to overload an operator, simply
/// register it.
///
/// ~~~~~~~~{.cpp}
/// class MyClass {
...
///        {fun(static_cast<std::string Test::*(double)>(&Test::functionOverload)),
///        "functionOverload"} {fun(static_cast<std::string Test::*(int)>(&Test::functionOverload)),
///        "functionOverload"} }
///      );
///
///
...
///
/// As much as possible, ChaiScript attempts to convert between &, *, const &, const *,
/// std::shared_ptr<T>, std::shared_ptr<const T>, std::reference_wrapper<T>,
/// std::reference_wrapper<const T> and value types automatically.
///
/// If a chaiscript::var object was created in C++ from a pointer, it cannot be converted to a
/// shared_ptr (this would add invalid reference counting). Const may be added, but never removed.
///
/// The take away is that you can pretty much expect function calls to Just Work when you need them
/// to.
///
/// ~~~~~~~~{.cpp}
/// void fun1(const int *);
...
/// ~~~~~~~~
///
/// See the unit test unittests/boxed_cast_test.cpp for a complete breakdown of the automatic casts
/// that available and tested.
///
/// -----------------------------------------------------------------------
///
/// @subsection baseclasses Base Classes
///
/// ChaiScript supports handling of passing a derived class object to a function expecting a base
/// class object. For the process to work, the base/derived relationship must be registered with the
/// engine.
///
/// ~~~~~~~~{.cpp}
/// class Base {};
...
///
/// As an alternative to the manual unboxing of exceptions shown above, exception specifications
/// allow the user to tell ChaiScript what possible exceptions are expected from the script being
/// executed.
///
/// Example:
/// ~~~~~~~~{.cpp}
...
///
/// try {
///   chai.eval("throw(runtime_error(@"error@"))", chaiscript::exception_specification<int, double,
///   float, const std::string &, const std::exception &>());
...
/// ~~~~~~~~
///
/// Operators can also be treated as functions by using the back tick operator. Building on the
/// above example:
///
/// ~~~~~~~~
/// eval> callfunc(`+`, 1, 4);
...
///
/// ~~~~~~~~
/// Function Definition ::= "def" identifier "(" [[type] arg ("," [type] arg)*] ")" [":" guard]
/// block Method Definition ::= "def" class_name "::" method_name "(" [[type] arg ("," [type] arg)*]
/// ")" [":" guard] block
/// ~~~~~~~~
///
...
///
/// Method definitions for known types extend those types with new methods. This includes C++ and
/// ChaiScript defined types. Method definitions for unknown types implicitly define the named type.
///
/// @sa @ref LangObjectSystemRef
///
///
...
/// @brief Namespace chaiscript contains every API call that the average user will be concerned
/// with.

/// @namespace chaiscript::detail
/// @brief Classes and functions reserved for internal use. Items in this namespace are not
/// supported.
    ...
    public:
        ChaiScript(std::vector<std::string> t_modulepaths = {},
            ...
            : ChaiScript_Basic(
                ...
                std::make_unique<
                    parser::ChaiScript_Parser<eval::Noop_Tracer, optimizer::Optimizer_Default> >(),
                t_modulepaths,
                t_usepaths,
                t_opts) {
        }
