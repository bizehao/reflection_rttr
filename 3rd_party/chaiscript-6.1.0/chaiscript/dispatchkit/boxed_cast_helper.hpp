        ...
        struct Cast_Helper_Inner<const Result *> {
            static const Result *cast(const Boxed_Value &ob, const Type_Conversions_State *) {
                return static_cast<const Result *>(
                    verify_type_no_throw(ob, typeid(Result), ob.get_const_ptr()));
            }
        };
...
/// \todo Fix the fact that this has to be in a shared_ptr for now
        ...
        struct Cast_Helper_Inner<const std::shared_ptr<Result> &>
                : Cast_Helper_Inner<std::shared_ptr<Result> > {
        };

        template<typename Result>
        struct Cast_Helper_Inner<std::shared_ptr<Result> &> {
            static_assert(!std::is_const<Result>::value,
                          "Non-const reference to std::shared_ptr<const T> is not supported");
        ...
        struct Cast_Helper_Inner<const std::shared_ptr<const Result>>
                : Cast_Helper_Inner<std::shared_ptr<const Result> > {
        };
        ...
        template<>
        struct Cast_Helper_Inner<Boxed_Value &> {
            static std::reference_wrapper<Boxed_Value> cast(const Boxed_Value &ob,
                                                            const Type_Conversions_State *) {
        ...
        struct Cast_Helper_Inner<const std::reference_wrapper<const Result>>
                : Cast_Helper_Inner<const Result &> {
        };
