...
namespace chaiscript {
    ...
    namespace dispatch {
        ...
        public:
            ...
            std::vector<Boxed_Value> convert(std::vector<Boxed_Value> vals,
                                ...
                                if (!bv.get_type_info().bare_equal(ti)) {
                                    if (t_conversions->converts(ti, bv.get_type_info())) {
                                        try {
                                            ...
                                            // let the user get it either way, we are not responsible if it
                                            // doesn't work
                                            vals[i] =
                                                    t_conversions->boxed_type_conversion(m_types[i].second,
                                                        t_conversions.saves(),
                                                        vals[i]);
                                        }
                                        catch (...) {
 ...
 */
    ...
    //! to the passed in values
            bool filter(const std::vector<Boxed_Value> &vals,
                        const Type_Conversions_State &t_conversions) const {
            ...
            static bool compare_type_to_param(const Type_Info &ti,
            ...
            virtual bool compare_first_type(const Boxed_Value &bv,
                                            const Type_Conversions_State &t_conversions) const {
        ...
        protected:
            ...
            static bool compare_types(const std::vector<Type_Info> &tis,
                                      const std::vector<Boxed_Value> &bvs,
                                      const Type_Conversions_State &t_conversions) {
    ...
    /// \brief Const version of Proxy_Function. Points to a const Proxy_Function. This is how most
 ...
 */
        ...
        public:
            ...
            bool operator==(const Proxy_Function_Base &rhs) const override {
                ...
                return this == &rhs
                       || ((prhs != nullptr) && this->m_arity == prhs->m_arity && !this->m_guard
                           && !prhs->m_guard && this->m_param_types == prhs->m_param_types);
            }

            bool call_match(const std::vector<Boxed_Value> &vals,
        ...
        protected:
            bool test_guard(const std::vector<Boxed_Value> &params,
                            const Type_Conversions_State &t_conversions) const {
                if (m_guard) {
                    try {
            ...
            std::pair<bool, bool> call_match_internal(const std::vector<Boxed_Value> &vals,
                ...
                return std::make_pair(comparison_result.first && test_guard(vals, t_conversions),
        ...
        public:
            Dynamic_Proxy_Function_Impl(Callable t_f,
                                        ...
                                        Param_Types t_param_types = Param_Types(),
                                        Proxy_Function t_guard = Proxy_Function())
                : Dynamic_Proxy_Function(t_arity,
        ...
        Proxy_Function make_dynamic_proxy_function(Callable &&c, Arg &&... a) {
            return chaiscript::make_shared<dispatch::Proxy_Function_Base,
...
/// it is used for bind(function, param1, _, param2) style calls
            ...
            std::vector<Boxed_Value> build_param_list(const std::vector<Boxed_Value> &params) const {
                ...
                while (!(parg == params.end() && barg == m_args.end())) {
                    while (barg != m_args.end()
                           && !(barg->get_type_info()
                                == chaiscript::detail::Get_Type_Info<Placeholder_Object>::get())) {
                        args.push_back(*barg);
                        ++barg;
                    }
        ...
        public:
            ...
            bool operator==(const Proxy_Function_Base &t_func) const override {
                return dynamic_cast<const Proxy_Function_Callable_Impl<Func, Callable> *>(&t_func)
                       != nullptr;
            }
        ...
        protected:
            Boxed_Value do_call(const std::vector<Boxed_Value> &params,
                                const Type_Conversions_State &t_conversions) const override {
                return detail::call_func(detail::Function_Signature<Func>(),
                                         m_f.get(),
                                         params,
                                         t_conversions);
            }
        ...
        public:
            ...
            bool call_match(const std::vector<Boxed_Value> &vals,
                            const Type_Conversions_State &) const override {
                if (vals.size() != 1) {
                    return false;
                }
        ...
        private:
            ...
            auto do_call_impl(const Class *o) const
                -> std::enable_if_t<std::is_pointer<Type>::value, Boxed_Value> {
            ...
            auto do_call_impl(Class *o) const
                -> std::enable_if_t<!std::is_pointer<Type>::value, Boxed_Value> {
                return detail::Handle_Return<typename std::add_lvalue_reference<Type>::type>::handle(
                    o->*m_attr);
            }
                    ...
                    if (types_match_except_for_arithmetic(begin->second, plist, t_conversions)) {
                        if (matching_func == end) {
                            ...
                            if (plist[0].is_const() && !mat_fun_param_types[1].is_const()
                                ...
                                matching_func =
                                        begin; // keep the new one, the const/non-const matchup is correct
                            }
