...
namespace chaiscript {
                                ...
                                [&]() {
                                    if (auto data = std::dynamic_pointer_cast<To>(
                                        ...
                                        if (std::string(typeid(To).name()).find("Assignable_Proxy_Function") !=
                                            std::string::npos) {
                                            auto from = detail::Cast_Helper<std::shared_ptr<From> >::cast(
                                                t_from, nullptr);
    ...
    public:
        ...
        Boxed_Value boxed_type_conversion(const Type_Info &to, Conversion_Saves &t_saves,
    ...
    private:
            ...
            return std::find_if(m_conversions.begin(), m_conversions.end(),
                                [&to, &from](const std::shared_ptr<detail::Type_Conversion_Base> &conversion) -> bool {
                                    return (conversion->to().bare_equal(to) && conversion->from().bare_equal(from))
                                           || (conversion->bidir() && conversion->from().bare_equal(to) && conversion->
                                               to().bare_equal(from));
                                }
            );
        }
        ...
        mutable chaiscript::detail::threading::Thread_Storage<std::set<const std::type_info *, Less_Than> >
        m_thread_cache;
  ...
  ///
    ...
    Type_Conversion base_class(
        typename std::enable_if<std::is_polymorphic<Base>::value && std::is_polymorphic<Derived>::value>::type * =
                nullptr) {
    ...
    Type_Conversion type_conversion(const Type_Info &t_from, const Type_Info &t_to,
        ...
        return chaiscript::make_shared<detail::Type_Conversion_Base, detail::Type_Conversion_Impl<Callable> >(
            t_from, t_to, t_func);
    }
    ...
    Type_Conversion vector_conversion() {
        auto func = [](const Boxed_Value &t_bv) -> Boxed_Value {
            const std::vector<Boxed_Value> &from_vec = detail::Cast_Helper<const std::vector<Boxed_Value> &>::cast(
                t_bv, nullptr);
    ...
    Type_Conversion map_conversion() {
        auto func = [](const Boxed_Value &t_bv) -> Boxed_Value {
            const std::map<std::string, Boxed_Value> &from_map = detail::Cast_Helper<const std::map<std::string,
                Boxed_Value> &>::cast(t_bv, nullptr);

            To map;
            for (const std::pair<std::string, Boxed_Value> &p: from_map) {
                map.insert(std::make_pair(
                    p.first, detail::Cast_Helper < typename To::mapped_type > ::cast(p.second, nullptr)));
            }
