...
namespace chaiscript {
    namespace optimizer {
        ...
        const eval::AST_Node_Impl<T> &child_at(const eval::AST_Node_Impl<T> &node, const size_t offset) {
            if (node.children[offset]->identifier == AST_Node_Type::Compiled) {
                return *(dynamic_cast<const eval::Compiled_AST_Node<T> &>(*node.children[offset])
                    .m_original_node);
            }
            else {
                return *node.children[offset];
    ...
    eval::Compiled_AST_Node<T>&>(*node).m_original_node->children[offset]; } else { return
    ...
    */
        ...
        auto child_count(const eval::AST_Node_Impl<T> &node) {
            if (node.identifier == AST_Node_Type::Compiled) {
                return dynamic_cast<const eval::Compiled_AST_Node<T> &>(node)
                        .m_original_node->children.size();
            }
            else {
        ...
        auto make_compiled_node(eval::AST_Node_Impl_Ptr<T> original_node,
                                std::vector<eval::AST_Node_Impl_Ptr<T> > children,
                                Callable callable) {
            return chaiscript::make_unique<eval::AST_Node_Impl<T>, eval::Compiled_AST_Node<T> >(
                std::move(original_node),
                std::move(children),
                std::move(callable));
        }
                    ...
                    if (!contains_var_decl_in_scope(*node)) {
                        if (node->children.size() == 1) {
                            ...
                            return chaiscript::make_unique<eval::AST_Node_Impl<T>,
                                eval::Scopeless_Block_AST_Node<T> >(
                                node->text,
                                node->location,
                ...
                if ((node->identifier == AST_Node_Type::Block
                    ...
                    for (size_t i = 0; i < node->children.size() - 1; ++i) {
                        ...
                        if (child->identifier == AST_Node_Type::Fun_Call) {
                            node->children[i] =
                                    chaiscript::make_unique<eval::AST_Node_Impl<T>,
                                        eval::Unused_Return_Fun_Call_AST_Node<T> >(
                                        child->text,
                                        child->location,
        ...
        struct Assign_Decl {
            template<typename T>
            auto optimize(eval::AST_Node_Impl_Ptr<T> node) {
                if ((node->identifier == AST_Node_Type::Equation) && node->text == "="
            ...
            auto optimize(eval::AST_Node_Impl_Ptr<T> node) {
                if ((node->identifier == AST_Node_Type::If) && node->children.size() >= 2
                    ...
                    const auto condition =
                            dynamic_cast<eval::Constant_AST_Node<T> *>(node->children[0].get())->m_value;
                ...
                if (node->identifier == AST_Node_Type::Binary && node->children.size() == 2
                    ...
                    try {
                        ...
                        if (parsed != Operators::Opers::invalid) {
                            const auto rhs =
                                    dynamic_cast<eval::Constant_AST_Node<T> *>(node->children[1].get())->m_value;
                            if (rhs.get_type_info().is_arithmetic()) {
                                return chaiscript::make_unique<
                                    eval::AST_Node_Impl<T>,
                                    eval::Fold_Right_Binary_Operator_AST_Node<T> >(node->text,
                                    node->location,
                                    std::move(node->children),
                                    rhs);
                            }
            ...
            auto optimize(eval::AST_Node_Impl_Ptr<T> node) {
                if (node->identifier == AST_Node_Type::Prefix && node->children.size() == 1
                    ...
                    try {
                        ...
                        const auto lhs =
                                dynamic_cast<const eval::Constant_AST_Node<T> *>(node->children[0].get())
                                ->m_value;
                        ...
                        if (parsed != Operators::Opers::invalid && parsed != Operators::Opers::bitwise_and
                            ...
                            return chaiscript::make_unique<eval::AST_Node_Impl<T>,
                                eval::Constant_AST_Node<T> >(std::move(match),
                                                             node->location,
                                                             std::move(val));
                        }
                        else if (lhs.get_type_info().bare_equal_type_info(typeid(bool)) && oper == "!") {
                            return chaiscript::make_unique<eval::AST_Node_Impl<T>,
                                eval::Constant_AST_Node<T> >(
                                std::move(match),
                                node->location,
                ...
                else if ((node->identifier == AST_Node_Type::Logical_And
                          || node->identifier == AST_Node_Type::Logical_Or)
                         && node->children.size() == 2
                         && node->children[0]->identifier == AST_Node_Type::Constant
                         && node->children[1]->identifier == AST_Node_Type::Constant) {
                    try {
                        const auto lhs =
                                dynamic_cast<const eval::Constant_AST_Node<T> &>(*node->children[0]).m_value;
                        const auto rhs =
                                dynamic_cast<const eval::Constant_AST_Node<T> &>(*node->children[1]).m_value;
                        if (lhs.get_type_info().bare_equal_type_info(typeid(bool))
                            && rhs.get_type_info().bare_equal_type_info(typeid(bool))) {
                            const auto match =
                                    node->children[0]->text + " " + node->text + " " + node->children[1]->text;
                            const auto val = [lhs_val = boxed_cast<bool>(lhs),
                                rhs_val = boxed_cast<bool>(rhs),
                                id = node->identifier] {
                    ...
                    try {
                        ...
                        if (parsed != Operators::Opers::invalid) {
                            ...
                            if (lhs.get_type_info().is_arithmetic() && rhs.get_type_info().is_arithmetic()) {
                                ...
                                const auto match =
                                        node->children[0]->text + " " + oper + " " + node->children[1]->text;
                                return chaiscript::make_unique<eval::AST_Node_Impl<T>,
                                    eval::Constant_AST_Node<T> >(std::move(match),
                                                                 node->location,
                                                                 std::move(val));
                    ...
                    const auto arg =
                            dynamic_cast<const eval::Constant_AST_Node<T> &>(*node->children[1]->children[0])
                            .m_value;
            ...
            auto optimize(eval::AST_Node_Impl_Ptr<T> for_node) {
                ...
                if (child_count(*for_node) == 4 && eq_node.identifier == AST_Node_Type::Assign_Decl
                    ...
                    const Boxed_Value &begin =
                            dynamic_cast<const eval::Constant_AST_Node<T> &>(child_at(eq_node, 1)).m_value;
                    const Boxed_Value &end =
                            dynamic_cast<const eval::Constant_AST_Node<T> &>(child_at(binary_node, 1)).m_value;
                    ...
                    if (begin.get_type_info().bare_equal(user_type<int>())
                        ...
                        return make_compiled_node(
                            ...
                            [id, start_int, end_int](
                        const std::vector<eval::AST_Node_Impl_Ptr<T> > &children,
                                ...
                                try {
                                    for (; i < end_int; ++i) {
                                        try {
                                            ...
                                            // loop implementation is skipped and we just need to continue
        ...
        typedef Optimizer<optimizer::Partial_Fold,
