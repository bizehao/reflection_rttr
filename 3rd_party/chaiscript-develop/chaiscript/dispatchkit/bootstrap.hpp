...
namespace chaiscript {
                      ...
                      [extent](T &t, size_t index)-> ReturnType & {
                          if (extent > 0 && index >= extent) {
                              throw std::range_error(
                                  "Array index out of range. Received: " + std::to_string(index) + " expected < " +
    ...
    /// Similar to shared_ptr_clone. Used for Proxy_Function.
        ...
        Boxed_Value ptr_assign(Boxed_Value lhs, const std::shared_ptr<Type> &rhs) {
            if (lhs.is_undef()
                || (!lhs.get_type_info().is_const() && lhs.get_type_info().bare_equal(
                        chaiscript::detail::Get_Type_Info<Type>::get()))) {
    ...
    /// from this class.
      ...
      /// \returns passed in Module
            static void bootstrap(Module &m) {
                ...
                m.add(fun(return_boxed_value_vector(&dispatch::Proxy_Function_Base::get_param_types)),
                      "get_param_types");
                ...
                m.add(fun(static_cast<Boxed_Value & (dispatch::Dynamic_Object::*)(const std::string &)>(&
                          dispatch::Dynamic_Object::get_attr)), "get_attr");
                m.add(fun(static_cast<const Boxed_Value & (dispatch::Dynamic_Object::*)(const std::string &) const>(&
                          dispatch::Dynamic_Object::get_attr)), "get_attr");

                m.add(
                    fun(static_cast<Boxed_Value & (dispatch::Dynamic_Object::*)(const std::string &)>(&
                        dispatch::Dynamic_Object::method_missing)), "method_missing");
                m.add(
                    fun(static_cast<const Boxed_Value & (dispatch::Dynamic_Object::*)(const std::string &) const>(&
                        dispatch::Dynamic_Object::method_missing)), "method_missing");
          ...
          def `==`(Dynamic_Object lhs, Dynamic_Object rhs) : lhs.get_type_name() == rhs.get_type_name()
          {
            ...
            if (rhs_attrs.size() != lhs_attrs.size()) {
              ...
              return all_of(rhs_attrs, fun[lhs](x) { lhs.has_attr(x.first) && lhs.get_attr(x.first) == x.second; } );
                ...
                m.add(fun(
                          [](dispatch::Assignable_Proxy_Function &t_lhs,
                             const std::shared_ptr<const dispatch::Proxy_Function_Base> &t_rhs) {
                              t_lhs.assign(t_rhs);
                          }
                      ), "="
                );
                ...
                chaiscript::utility::add_class<AST_Node>(m,
                                                         ...
                                                         {},
                                                         {
                                                             ...
                                                             {
                                                                 fun([](
                                                                 const chaiscript::AST_Node &t_node) -> std::vector<
                                                                 Boxed_Value> {
                                                                         std::vector<Boxed_Value> retval;
