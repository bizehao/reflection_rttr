...
#pragma GCC diagnostic ignored "-Wfloat-conversion"
    ...
    private:
        ...
        static inline void check_divide_by_zero(
            T t, typename std::enable_if<std::is_integral<T>::value>::type * = nullptr) {
        ...
        static constexpr Common_Types get_common_type(size_t t_size, bool t_signed) {
            return (t_size == 1 && t_signed)
                       ? (Common_Types::t_int8)
                       : (t_size == 1)
                             ? (Common_Types::t_uint8)
                             : (t_size == 2 && t_signed)
                                   ? (Common_Types::t_int16)
                                   : (t_size == 2)
                                         ? (Common_Types::t_uint16)
                                         : (t_size == 4 && t_signed)
                                               ? (Common_Types::t_int32)
                                               : (t_size == 4)
                                                     ? (Common_Types::t_uint32)
                                                     : (t_size == 8 && t_signed)
                                                           ? (Common_Types::t_int64)
        ...
        static auto go(Operators::Opers t_oper, const Boxed_Value &t_lhs, const Boxed_Value &t_rhs)
            ...
            if (t_oper > Operators::Opers::boolean_flag && t_oper < Operators::Opers::non_const_flag) {
                ...
                return binary_go(t_oper, *static_cast<LHS *>(t_lhs.get_ptr()), get_as_aux<common_type, RHS>(t_rhs),
                                 t_lhs);
            }
                ...
                return const_binary_go(t_oper, get_as_aux<common_type, LHS>(t_lhs),
                                       get_as_aux<common_type, RHS>(t_rhs));
            }
            else {
                throw chaiscript::detail::exception::bad_any_cast();
        ...
        static auto go(Operators::Opers t_oper, const Boxed_Value &t_lhs)
            ...
            if (t_oper > Operators::Opers::non_const_flag && t_oper < Operators::Opers::non_const_int_flag && !t_lhs.
                is_const() && !t_lhs.is_return_value()) {
        ...
        template<typename LHS>
        inline static Boxed_Value oper_rhs(Operators::Opers t_oper, const Boxed_Value &t_lhs,
