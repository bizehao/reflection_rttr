  ...
  ///        Used to implement loadable module support.
    ...
    /// of the object stack, functions and registered types.
        ...
        /// ensure that it is always in scope.
            Boxed_Value get_object(const std::string &name, std::atomic_uint_fast32_t &t_loc,
                ...
                if (loc == 0) {
                    ...
                    return stack[stack.size() - 1 - ((loc & static_cast<uint_fast32_t>(Loc::stack_mask)) >> 16)][
                        loc & static_cast<uint_fast32_t>(Loc::loc_mask)].second;
                }
        ...
        /// match
            ...
            std::pair<size_t, std::shared_ptr<std::vector<Proxy_Function> > > get_function(
                const std::string &t_name, const size_t t_hint) const {
                chaiscript::detail::threading::shared_lock<chaiscript::detail::threading::shared_mutex> l(m_mutex);
        ...
        /// \warn does not obtain a mutex lock. \sa get_function_object for public version
            std::pair<size_t, Boxed_Value>
            get_function_object_int(const std::string &t_name, const size_t t_hint) const {
        ...
        ///
            ...
            static bool is_attribute_call(const std::vector<Proxy_Function> &t_funs,
                ...
                return std::any_of(std::begin(t_funs), std::end(t_funs),
                                   [&](const auto &fun) {
                                       return fun->is_attribute_function() && fun->compare_first_type(
                                                  t_params[0], t_conversions);
                                   }
                );
            }
...
#pragma warning(disable : 4715)
            ...
            Boxed_Value call_member(const std::string &t_name, std::atomic_uint_fast32_t &t_loc,
                ...
                const auto do_attribute_call =
                        [this](int l_num_params, const std::vector<Boxed_Value> &l_params,
                    ...
                    if (l_num_params < int(l_params.size()) || bv.get_type_info().bare_equal(
                    ...
                    if (!functions.empty()) {
                        try {
                            if (is_no_param) {
                                ...
                                return dispatch::dispatch(functions, {
                                                              params[0], var(t_name),
                                                              var(std::vector<Boxed_Value>(
...
#pragma warning(pop)
        ...
        private:
            ...
            const std::vector<std::pair<std::string, std::shared_ptr<std::vector<Proxy_Function> > > > &
            get_functions_int() const {
            ...
            static typename Container::const_iterator find_keyed_value(const Container &t_c, const Key &t_key,
                ...
                if (t_c.size() > t_hint && t_c[t_hint].first == t_key) {
                    return std::next(t_c.begin(),
                                     static_cast<typename std::iterator_traits<typename
                                         Container::const_iterator>::difference_type>(t_hint));
                }
                else {
                ...
                Proxy_Function new_func =
                        [&]() -> Proxy_Function {
                            if (itr != funcs.end()) {
                                ...
                                funcs.emplace_back(
                                    t_name, std::make_shared<std::vector<Proxy_Function> >(
