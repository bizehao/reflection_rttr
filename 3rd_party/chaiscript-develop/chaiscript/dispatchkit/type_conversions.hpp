...
namespace chaiscript {
            ...
            static Boxed_Value cast(const Boxed_Value &t_from) {
                if (t_from.get_type_info().bare_equal(chaiscript::user_type<From>())) {
                    if (t_from.is_pointer()) {
                            ...
                            return Boxed_Value(
                                [&]() {
                                    if (auto data = std::static_pointer_cast<const To>(
                                        detail::Cast_Helper<std::shared_ptr<const From> >::cast(
                                            t_from,
                                            nullptr))) {
                                        return data;
                                    }
                        ...
                        // Pull the reference out of the contained boxed value, which we know is the type we
                        // want
                        if (t_from.is_const()) {
                    ...
                    throw chaiscript::exception::bad_boxed_dynamic_cast(t_from.get_type_info(),
                                                                        typeid(To),
                            ...
                            return Boxed_Value(
                                [&]() {
                                    if (auto data = std::dynamic_pointer_cast<To>(
                                        ...
                                        if (std::string(typeid(To).name()).find("Assignable_Proxy_Function")
                                            != std::string::npos) {
                                            auto from =
                                                    detail::Cast_Helper<std::shared_ptr<From> >::cast(t_from,
                                                        nullptr);
                                            if (std::string(typeid(*from).name())
                                                .find("Assignable_Proxy_Function_Impl")
                                                != std::string::npos) {
    ...
    public:
        ...
        void add_conversion(const std::shared_ptr<detail::Type_Conversion_Base> &conversion) {
            chaiscript::detail::threading::unique_lock<chaiscript::detail::threading::shared_mutex> l(
                m_mutex);
            /// \todo error if a conversion already exists
            m_conversions.insert(conversion);
            m_convertableTypes.insert(
                {conversion->to().bare_type_info(), conversion->from().bare_type_info()});
        ...
        Boxed_Value boxed_type_conversion(const Type_Info &to,
            ...
            try {
                ...
                throw exception::bad_boxed_dynamic_cast(from.get_type_info(),
                                                        *to.bare_type_info(),
                                                        "No known conversion");
            }
        ...
        Boxed_Value boxed_type_down_conversion(const Type_Info &from,
            ...
            try {
                ...
                throw exception::bad_boxed_dynamic_cast(to.get_type_info(),
                                                        *from.bare_type_info(),
                                                        "No known conversion");
            }
            catch (const std::bad_cast &) {
        ...
        bool has_conversion(const Type_Info &to, const Type_Info &from) const {
            chaiscript::detail::threading::shared_lock<chaiscript::detail::threading::shared_mutex> l(
                m_mutex);
            return find_bidir(to, from) != m_conversions.end();
        }

        std::shared_ptr<detail::Type_Conversion_Base> get_conversion(const Type_Info &to,
                                                                     const Type_Info &from) const {
            chaiscript::detail::threading::shared_lock<chaiscript::detail::threading::shared_mutex> l(
                m_mutex);
            ...
            if (itr != m_conversions.end()) {
                ...
                throw std::out_of_range("No such conversion exists from " + from.bare_name() + " to "
                                        + to.bare_name());
            }
        }
    ...
    private:
        ...
        mutable chaiscript::detail::threading::Thread_Storage<
            std::set<const std::type_info *, Less_Than> >
        m_thread_cache;
...
///
    ...
    Type_Conversion base_class(
        ...
        return chaiscript::make_shared<detail::Type_Conversion_Base,
            detail::Dynamic_Conversion_Impl<Base, Derived> >();
    }
    ...
    Type_Conversion base_class(
        ...
        return chaiscript::make_shared<detail::Type_Conversion_Base,
            detail::Static_Conversion_Impl<Base, Derived> >();
    }

    template<typename Callable>
    Type_Conversion type_conversion(const Type_Info &t_from,
                                    const Type_Info &t_to,
                                    const Callable &t_func) {
        return chaiscript::make_shared<detail::Type_Conversion_Base,
            detail::Type_Conversion_Impl<Callable> >(t_from, t_to, t_func);
    }
    ...
    Type_Conversion type_conversion(const Callable &t_function) {
        auto func = [t_function](const Boxed_Value &t_bv) -> Boxed_Value {
            ...
            return chaiscript::Boxed_Value(
                t_function(detail::Cast_Helper<const From &>::cast(t_bv, nullptr)));
        };

        return chaiscript::make_shared<detail::Type_Conversion_Base,
            detail::Type_Conversion_Impl<decltype(func)> >(user_type<From>(),
                                                           user_type<To>(),
                                                           func);
    }
    ...
    Type_Conversion vector_conversion() {
        ...
        return chaiscript::make_shared<detail::Type_Conversion_Base,
            ...
            user_type<std::vector<Boxed_Value> >(),
            user_type<To>(),
            func);
    }
    ...
    Type_Conversion map_conversion() {
        auto func = [](const Boxed_Value &t_bv) -> Boxed_Value {
            ...
            for (const std::pair<std::string, Boxed_Value> &p: from_map) {
                map.insert(std::make_pair(
                    p.first,
                    detail::Cast_Helper < typename To::mapped_type > ::cast(p.second, nullptr)));
            }
        ...
        return chaiscript::make_shared<detail::Type_Conversion_Base,
            detail::Type_Conversion_Impl<decltype(func)> >(
            user_type<std::map<std::string, Boxed_Value> >(),
            user_type<To>(),
            func);
    }
} // namespace chaiscript
