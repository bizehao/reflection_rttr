...
namespace chaiscript {
    ...
    class ChaiScript_Basic {
        ...
        void build_eval_system(const ModulePtr &t_lib, const std::vector<Options> &t_opts) {
            ...
            m_engine.add(fun([this](const Boxed_Value &t_bv, const std::string &t_type) {
                return m_engine.is_type(t_bv, t_type);
            }), "is_type");
            m_engine.add(fun([this](const Boxed_Value &t_bv) { return m_engine.type_name(t_bv); }), "type_name");
            m_engine.add(fun([this](const std::string &t_f) { return m_engine.function_exists(t_f); }),
                         "function_exists");
            ...
            if (std::find(t_opts.begin(), t_opts.end(), Options::No_External_Scripts) == t_opts.end()
                ...
                m_engine.add(fun([this](const std::string &t_file) { return internal_eval_file(t_file); }),
                             "eval_file");
            }
            ...
            m_engine.add(fun([this](const Boxed_Value &t_bv, const std::string &t_name) {
            ...
            m_engine.add(fun([this](const std::string &t_namespace_name) {
                register_namespace([](Namespace & /*space*/) {
                }, t_namespace_name);
    ...
    /// \throw chaiscript::exception::load_module_error In the event that no matching module can be found.
        std::string load_module(const std::string &t_module_name) {
            ...
            throw chaiscript::exception::load_module_error(
                "Loadable module support was disabled (CHAISCRIPT_NO_DYNLOAD)");
#else
    ...
    /// \throw std::runtime_error In the case that the namespace name was already registered.
        void register_namespace(const std::function<void(Namespace &)> &t_namespace_generator,
            ...
            if (!m_namespace_generators.count(t_namespace_name)) {
                ...
                m_namespace_generators.emplace(std::make_pair(t_namespace_name,
