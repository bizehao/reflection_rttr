...
namespace chaiscript {
    ...
    namespace parser {
        ...
        namespace detail {
            enum Alphabet {
                symbol_alphabet = 0, keyword_alphabet, int_alphabet, float_alphabet, x_alphabet, hex_alphabet,
                b_alphabet, bin_alphabet, id_alphabet, white_alphabet, int_suffix_alphabet, float_suffix_alphabet,
                max_alphabet, lengthof_alphabet = 256
            };
        ...
        class ChaiScript_Parser final : public ChaiScript_Parser_Base {
            ...
            static std::array<std::array < bool, detail::lengthof_alphabet>
            ,
            detail::max_alphabet
            >
            build_alphabet() {
            ...
            static const std::array<std::array < bool, detail::lengthof_alphabet>
            ,
            detail::max_alphabet
            >
            &
            ...
            const std::array<std::array < bool, detail::lengthof_alphabet>
            ,
            detail::max_alphabet
            >
            &
            m_alphabet= create_alphabet
            ();
            ...
            void validate_object_name(const std::string &name) const {
                if (!Name_Validator::valid_object_name(name)) {
                    throw exception::eval_error("Invalid Object Name: " + name,
        ...
        public:
            ...
            void debug_print(const AST_Node &t, std::string prepend = "") const override {
                std::cout << prepend << "(" << ast_node_type_to_string(t.identifier) << ") " << t.text << " : " << t.
                        start().line << ", " << t.start().column << '\n';
            ...
            void build_match(size_t t_match_start, std::string t_text = "") {
                ...
                if (is_deep) {
                    new_children.assign(
                        std::make_move_iterator(m_match_stack.begin() + static_cast<int>(t_match_start)),
                ...
                if (m_position.has_more() && (*m_position == '0')) {
                    ...
                    if (m_position.has_more() && char_in_alphabet(*m_position, detail::x_alphabet)) {
                        ...
                        if (m_position.has_more() && char_in_alphabet(*m_position, detail::hex_alphabet)) {
                            ...
                            while (m_position.has_more() &&
                                   char_in_alphabet(*m_position, detail::int_suffix_alphabet)) {
                                ++m_position;
                            }
...
#pragma GCC diagnostic ignored "-Wtautological-compare"
                ...
                try {
                    ...
                    if (!unsigned_ && !long_ && u >= std::numeric_limits<int>::min() && u <= std::numeric_limits<
                            int>::max()) {
                        return const_var(static_cast<int>(u));
                    }
                    else if ((unsigned_ || base != 10) && !long_ && u >= std::numeric_limits<unsigned int>::min() && u
                             <= std::numeric_limits<unsigned int>::max()) {
                        return const_var(static_cast<unsigned int>(u));
                    }
                    else if (!unsigned_ && !longlong_ && u >= std::numeric_limits<long>::min() && u <=
                    ...
                    else if (!unsigned_ && u >= std::numeric_limits<long long>::min() && u <= std::numeric_limits<long
                                 long>::max()) {
                    ...
                    try {
                        ...
                        if (!longlong_ && u >= std::numeric_limits<unsigned long>::min() && u <= std::numeric_limits<
                                unsigned long>::max()) {
                            return const_var(static_cast<unsigned long>(u));
                        }
                        else {
                        ...
                        if (Hex_()) {
                            ...
                            m_match_stack.emplace_back(
                                make_node<eval::Constant_AST_Node<Tracer> >(
                                    std::move(match), start.line, start.col, std::move(bv)));
                            return true;
                        }
...
#pragma warning(disable : 4307)
                    ...
                    switch (text_hash) {
                        case utility::fnv1a_32("true"): {
                            m_match_stack.push_back(
                                make_node<eval::Constant_AST_Node<Tracer> >(
                                    std::move(text), start.line, start.col, const_var(true)));
                        }
                        break;
                        case utility::fnv1a_32("false"): {
                            m_match_stack.push_back(
                                make_node<eval::Constant_AST_Node<Tracer> >(
                                    std::move(text), start.line, start.col, const_var(false)));
                        }
                        break;
...
#pragma warning(pop)
            ...
            bool Quoted_String_() {
                if (m_position.has_more() && (*m_position == '\"')) {
                    ...
                    while (m_position.has_more() && (
                               (*m_position != '\"') || (in_interpolation > 0) || (prev_char == '\\'))) {
                        if (!Eol_()) {
                    ...
                    if (t_char == '\\') {
                        ...
                        if (is_escaped) {
                            if (is_octal_char) {
                                ...
                                switch (t_char) {
                                    case ('\''): match.push_back('\'');
                                        break;
                                    case ('\"'): match.push_back('\"');
                                        break;
                                    case ('?'): match.push_back('?');
                                        break;
                                    case ('a'): match.push_back('\a');
                                        break;
                                    case ('b'): match.push_back('\b');
                                        break;
                                    case ('f'): match.push_back('\f');
                                        break;
                                    case ('n'): match.push_back('\n');
                                        break;
                                    case ('r'): match.push_back('\r');
                                        break;
                                    case ('t'): match.push_back('\t');
                                        break;
                                    case ('v'): match.push_back('\v');
                                        break;
                                    case ('$'): match.push_back('$');
                                        break;
                                    default: throw exception::eval_error(
                                            "Unknown escaped sequence in string", File_Position(line, col), filename);
                                }
                                is_escaped = false;
                            }
                        }
            ...
            bool Single_Quoted_String() {
                ...
                if (Single_Quoted_String_()) {
                    ...
                    m_match_stack.push_back(
                        make_node<eval::Constant_AST_Node<Tracer> >(match, start.line, start.col,
                                                                    const_var(char(match.at(0)))));
                    return true;
                }
                else {
                    return false;
            ...
            bool Symbol(const utility::Static_String &t_s, const bool t_disallow_prevention = false) {
                ...
                if (retval && m_position.has_more() && (t_disallow_prevention == false) && char_in_alphabet(
                        *m_position, detail::symbol_alphabet)) {
                    if (*m_position != '=' && is_operator(Position::str(start, m_position)) && !is_operator(
                            Position::str(start, m_position + 1))) {
            ...
            bool Try() {
                ...
                if (Keyword("try")) {
                    ...
                    if (!Block()) {
                        throw exception::eval_error("Incomplete 'try' block",
                                                    File_Position(m_position.line, m_position.col), *m_filename);
                    }
            ...
            bool If() {
                ...
                if (Keyword("if")) {
                    ...
                    if (!Block()) {
                        throw exception::eval_error("Incomplete 'if' block",
                                                    File_Position(m_position.line, m_position.col), *m_filename);
                    }
            ...
            bool While() {
                ...
                if (Keyword("while")) {
                    ...
                    if (!Block()) {
                        throw exception::eval_error("Incomplete 'while' block",
                                                    File_Position(m_position.line, m_position.col), *m_filename);
                    }
            ...
            bool For() {
                ...
                if (Keyword("for")) {
                    ...
                    if (!Block()) {
                        throw exception::eval_error("Incomplete 'for' block",
                                                    File_Position(m_position.line, m_position.col), *m_filename);
                    }
            ...
            bool Case() {
                ...
                if (Keyword("case")) {
                    ...
                    if (!Block()) {
                        throw exception::eval_error("Incomplete 'case' block",
                                                    File_Position(m_position.line, m_position.col), *m_filename);
                    }
            ...
            bool Switch() {
                ...
                if (Keyword("switch")) {
                    ...
                    if (Char('{')) {
                        ...
                        if (!Char('}')) {
                            throw exception::eval_error("Incomplete block",
                                                        File_Position(m_position.line, m_position.col), *m_filename);
                        }
                    }
                    else {
                ...
                if (Lambda() || Num() || Quoted_String() || Single_Quoted_String() ||
                    ...
                    while (has_more) {
                        ...
                        if (Char('(')) {
                            ...
                            if (std::distance(m_match_stack.begin() + static_cast<int>(prev_stack_top),
                                              m_match_stack.end()) != 2) {
                                throw exception::eval_error("Incomplete dot access fun call",
                                                            File_Position(m_position.line, m_position.col),
                                                            *m_filename);
                    ...
                    if ((is_char && Char(oper.c_str()[0])) || (!is_char && Symbol(oper))) {
                        if (!Operator(m_operators.size() - 1)) {
                            throw exception::eval_error(
                                "Incomplete prefix '" + std::string(oper.c_str()) + "' expression",
                                File_Position(m_position.line, m_position.col), *m_filename);
                        }
            ...
            bool Map_Pair() {
                ...
                if (Operator()) {
                    if (Symbol(":")) {
                        retval = true;
                        if (!Operator()) {
                            throw exception::eval_error("Incomplete map pair",
                                                        File_Position(m_position.line, m_position.col), *m_filename);
                        }
