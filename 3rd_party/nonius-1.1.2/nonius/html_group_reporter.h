...
namespace nonius {
    ...
    struct html_group_reporter : reporter {
        void set_current_group_name(std::string benchmark_group, std::string tooltip = std::string()) {
            m_all_benchmarks.push_back(
                std::make_pair(group_data{benchmark_group, tooltip}, benchmark_list()));
        ...
        void generate_report() const {
            ...
            map["barchart_subtitle"] =
                    m_can_print_datetime ? escape(get_current_datetime()) : std::string("");
            ...
            for (const auto &bench_group: m_all_benchmarks) {
                ...
                for (const auto &bench_list: bench_group.second) {
                    ...
                    benchmark_data["mean"] =
                            bench_list.second.m_analysis.mean.point.count() * mean_magnitude;
                    benchmark_data["mean_unit"] = detail::units_for_magnitude(mean_magnitude);
                    benchmark_data["mean_lb"] =
                            bench_list.second.m_analysis.mean.lower_bound.count() * mean_magnitude;
                    benchmark_data["mean_lb_unit"] = detail::units_for_magnitude(mean_magnitude);
                    benchmark_data["mean_ub"] =
                            bench_list.second.m_analysis.mean.upper_bound.count() * mean_magnitude;
                    benchmark_data["mean_ub_unit"] = detail::units_for_magnitude(mean_magnitude);
                    benchmark_data["mean_ci"] =
                            truncate(bench_list.second.m_analysis.mean.confidence_interval);
                    ...
                    benchmark_data["std_dev"] =
                            bench_list.second.m_analysis.standard_deviation.point.count()
                            * std_dev_magnitude;
                    benchmark_data["std_dev_unit"] = detail::units_for_magnitude(std_dev_magnitude);
                    const auto std_dev_lb_magnitude = detail::get_magnitude(
                        bench_list.second.m_analysis.standard_deviation.lower_bound);
                    benchmark_data["std_dev_lb"] =
                            bench_list.second.m_analysis.standard_deviation.lower_bound.count()
                            * std_dev_lb_magnitude;
                    benchmark_data["std_dev_lb_unit"] =
                            detail::units_for_magnitude(std_dev_lb_magnitude);
                    const auto std_dev_ub_magnitude = detail::get_magnitude(
                        bench_list.second.m_analysis.standard_deviation.upper_bound);
                    benchmark_data["std_dev_ub"] =
                            bench_list.second.m_analysis.standard_deviation.upper_bound.count()
                            * std_dev_ub_magnitude;
                    benchmark_data["std_dev_ub_unit"] =
                            detail::units_for_magnitude(std_dev_ub_magnitude);
                    ...
                    benchmark_data["outliers_descr"] =
                            get_outliers_effect(bench_list.second.m_analysis);
...
void do_measurement_start(execution_plan<fp_seconds> plan) override {
    ...
    if (!summary)
        std::cout << "collecting " << n_samples << " samples, " << plan.iterations_per_sample
                << " iterations each, in estimated "
                << detail::pretty_duration(plan.estimated_duration) << "\n";
}
...
void do_analysis_complete(sample_analysis<fp_seconds> const &analysis) override {
    ...
    if (verbose)
        std::cout << "variance introduced by outliers: "
                << detail::percentage(analysis.outlier_variance) << "\n";
    const char *effect;
    if (analysis.outlier_variance < 0.01) {
        effect = "unaffected";
...
void print_environment_estimate(environment_estimate<fp_seconds> e, int iterations) {
    std::cout << std::setprecision(7);
    std::cout.unsetf(std::ios::floatfield);
    std::cout << "mean is " << detail::pretty_duration(e.mean) << " (" << iterations
            << " iterations)\n";
    if (verbose)
...
void print_outlier_count(const char *description, int count, int total) {
    if (count > 0)
        std::cout << "  " << count << " (" << detail::percentage_ratio(count, total) << ") "
                << description << "\n";
}
...
void print_statistic_estimate(const char *name, estimate<fp_seconds> estimate) {
    ...
    if (!summary) {
        std::cout << ", lb " << detail::pretty_duration(estimate.lower_bound) << ", ub "
                << detail::pretty_duration(estimate.upper_bound) << ", ci "
                << std::setprecision(3) << estimate.confidence_interval;
    }
    std::cout << "\n";
}
